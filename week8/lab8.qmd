...
```{r}
#| label: read-data
csv <- read_csv(here::here("week8", "wildfires", "wildfires.csv"))
shapefile <- st_read(here::here("week8", "wildfires", "wildfires.shp"))
tiff <- rast(here::here("week8", "wildfires", "vpd.tiff"))
...
```{r}
tiff_stack <- tiff[[c("1980", "2000", "2020")]]

ggplot() +
  geom_spatraster(data = tiff_stack) +
  facet_wrap(~lyr) +       # facet maps
  scale_fill_viridis_c(name = "VPD") +
  theme_minimal()
...
```{r}
ca <- ne_states(country = "United States of America",
                returnclass = "sf") |>
  dplyr::filter(name == "California")

shapefile_years <- shapefile |>
  filter(fire_year %in% c(1980, 2000, 2020))


ggplot() +
  geom_sf(data = ca, fill = "lightblue") +
  geom_sf(data = shapefile_years, aes(fill = acres), color = NA) +
  facet_wrap(~ fire_year, ncol = 3) +
  scale_fill_viridis_c(name = "Acres Burned") +
  theme_minimal()
...
ggplot(class_model_pred, aes(x = mean_vpd_kpa, y = lam)) +
  geom_ribbon(aes(ymin = lam_lwr, ymax = lam_upr), alpha = 0.2) +
  geom_line() +
  geom_point() +
  theme_classic()
...
```{r}
summary_wildfire_lm <- summary(wildfire_lm)
summary_wildfire_lm
```

```{r}
summary_poisson_model <- summary(poisson_model)
summary_poisson_model
```
...
```{r}
qnorm(c(.025, 0.975), mean = 5.30, 8.44)
```

    Why doesn't that interval make any sense?**
because we cant give negative fires 
