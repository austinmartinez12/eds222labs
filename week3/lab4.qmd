...
restoration <- read_csv(here::here("week3","eelgrass.csv")
...
HA: The popsicle stick method works better for restoration than garden staples
...
restoration_summary <- restoration |>
  group_by(treatment) |>
  summarise(prop_succsess = sum(success)/sum(n()))

x <- defferece$prop_succsess[2] - defferece$prop_succsess[1]
print(x)
```
...
permutation_props <- one_permutation %>% 
  group_by(treatment) %>% 
  summarize(prop_succsess = sum(success)/sum(n()))

permutation_diff_props <- permutation_props$prop_succsess[2] - permutation_props$prop_succsess[1]
...
df = data.frame(null_distribution)

df |>
ggplot(aes(x=null_distribution)) +
  geom_histogram()

ggplot(tibble(null_distribution), aes(null_distribution)) +
  geom_histogram(binwidth = 0.04) +
  geom_vline(xintercept = observed, color = "firebrick", linewidth = 2)
```
...
sd <- sd(df$null_distribution)
mean <- mean(df$null_distribution)

p_val <- (sd*2) + mean
p_val

pval <- sum(null_distribution >= observed) / length(null_distribution)
pval
```
