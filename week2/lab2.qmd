...
ggplot(data = storm_data, aes(y= gage_ht, x= precip_wk_in)) +
  geom_point()
...
```{r}
storm_sum$sigma
# How much we estimate or estimate to be off by
```

**Q2: What are the estimates for** $\hat \beta_0, \hat \beta_1, \hat \sigma$ **?**

beta 0 hat 3.9 beta 1 hat 2.2 sigma hat 2.3 \### Interpret model

**Q3: In plain language, describe how you'd interpret** $\beta_0$ **and** $\beta_1$**. What do they represent?**

Beta_0: estimate gage hight when no preciptaitoon

beta_1: for every inch of rain, the increase in gage height
...
# different componnents to estimate the squared

y_bar <- mean(storm_data$gage_ht)
x <- storm_data$precip_wk_in
y_hat <- 3.9 + 2.2 * x
#beta_0_hat <- coef(storm_mod) in the future we can do it like this
SST <- sum((storm_data$gage_ht - y_bar)^2)
SSE <- sum((storm_data$gage_ht - y_hat)^2)
#what fraction of the variance in gage height have er explained using precipitation?
R2 <- (SST - SSE) / SST

```

**Q4: What's the difference between** $y$**,** $\bar y$**, and** $\hat y$**?** y - observed gage height y bar - mean gage height (overall) y_hat - expected gage height (given precipitation)
