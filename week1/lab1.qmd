...
histograms show distribution of the data and bar plots show amount of data in a catigorical
...
```{r}
crayfish <- read_csv(here::here("week1", "crayfish.csv")) |> drop_na()
```
...
size <- crayfish |>
  group_by(Conc_Code) |>
  summarise(mean = mean(Length_mm, na.rm = TRUE))
```
...
crayfish |>
  count(Length_mm) |>
  group_by(Conc_Code) |>
  mutate(mean = mean(Length_mm))
```


```{r}
size_summary <- crayfish %>%
  group_by(Conc_Code) %>%
  summarise(mean_length = mean(Length_mm, na.rm = TRUE))
ggplot(size_summary, aes(x = Conc_Code, y = mean_length, fill = Conc_Code)) +
  geom_bar(stat = "identity") +
  labs(x = "Nickel Concentration", 
       y = "Mean Length (mm)",
       title = "Mean Crayfish Length by Nickel Concentration",
       fill = "Ni Level") +
  scale_fill_manual(
    values = c(
      "C" = "orange",
  ...
  coord_cartesian(ylim = c(15, 19))   # zoom in to exaggerate differences
