...
water_data <- read_csv(here::here("week7", "drinking_water.csv"))
...
| `pctpov`            | \% poverty                      |
| `pcthisp`           | \% hisapnic                     |
| `health`            | count of health code violations |
| `medianincomehouse` | Median income                   |
| `violation`         | if they got a ivolation         |
| `pcthisp_bin`       |                                 |
| `pctpov_bin`        |                                 |
...
library(patchwork)
fig1_data <- water_data %>%
  group_by(pcthisp_bin) %>%
  summarise(prop_violation = mean(violation, na.rm = TRUE))

p1 <- ggplot(fig1_data, aes(x = pcthisp_bin, y = prop_violation)) +
  geom_point(color = "darkred", size = 3) +
  labs(
    title = "Figure 1: Proportion of SDWA Violations by % Hispanic Bin",
    x = "% Hispanic (binned)",
    y = "Proportion with Violations"
  ) +
  theme_minimal()


fig2_data <- water_data %>%
  group_by(pctpov_bin) %>%
  summarise(prop_violation = mean(violation, na.rm = TRUE))

p2 <-  ggplot(fig2_data, aes(x = pctpov_bin, y = prop_violation)) +
  geom_point(color = "steelblue", size = 3) +
  labs(
    title = "Figure 2: Proportion of SDWA Violations by % Below Poverty Bin",
...
p3 <- ggplot(water_data, aes(x = pcthisp, y = pctpov)) +
  geom_point(alpha = 0.6, color = "forestgreen") +
  geom_smooth(method = "lm", se = FALSE, color = "black") +
  labs(
    title = "Figure 3: Correlation Between % Hispanic and % Below Poverty",
    x = "% Hispanic",
    y = "% Below Poverty"
  ) +
  theme_minimal()
...
class_model <- glm(violation ~ pctpov, # response ~ predictor
                   data = water_data,
                   family = binomial(link = "logit"))
...
**Based on the sign, what is the relationship between class and SDWA violations?** sign is positive so a positive association between the two
...
# grid of predictors to generate predictoins for
pred_grid <- expand_grid(pctpov = 0:100)
# Generate our predictions
class_model_red <- pred_grid |>
...
ggplot(class_model_red, aes(x = pctpov, y=p)) +
  geom_line() +
  ylim(0,1)
...
class_model_se <- predict(object = class_model,
                          newdata = pred_grid,
                          type = "link", # stay in line space
                          se.fit = TRUE)# get the SE fit

# go from link space to response space
linkinv <- family(class_model)$linkinv # undose logit
class_model <- pred_grid |>
  mutate(
    # get logit(p)
    logit_p = class_model_se$fit,
    logit_p_se = class_model_red$se.fit,
    logit_p_lwr = qnorm(0.025, mean = logit_p, sd = logit_p_se),
    logit_p_upr = qnorm(0.975, mean = logit_p, sd = logit_p_se),
    p = linkinv(logit_p_lwr),
...
```{r}
# gpt version
linkinv <- family(class_model)$linkinv # undo the logit
...
#| label: plot-class-ci
#| 
ggplot(class_model, aes(x = pctpov, y = p)) +
...
```{r}
# 1.  Create a new model with an interaction term between `pctpov` and `pcthisp`.
class_model2 <- glm(violation ~ pctpov * pcthisp, # response ~ predictor
                   data = water_data,
                   family = binomial(link = "logit"))
```
...
```{r}
#95% CI for the % Hispanic coefficient
confint(class_model2)["pcthisp",]
```
The 95% CI for the % Hispanic coefficient is:
       2.5 %       97.5 % 
0.0001928585 0.0004555946 


  **Is the interaction between class and race significantly different than 0?**
```{r}
coef(class_model2)["pctpov:pcthisp"]
```
Yes, the relationship has a p-value of 1.25 × 10⁻⁶. SO, it is much smaller than 0.05. Meaning the interaction between poverty and Hispanic population is statistically significant.

3.  Create a new predictor data frame with combinations of *both* predictors.
```{r}
pred_grid2 <- expand_grid(pctpov = 0:100,
                         pcthisp = 0:100)
```
4.  Generate predictions, including both the best estimate and 95% CI.
```{r}
class_model_pred2 <- pred_grid2 |>
...
pred <- predict(
  class_model2,
  newdata = pred_grid2,
  type = "link",      # predict on link (logit) scale for CI
  se.fit = TRUE
)

# Add predictions + 95% CI back into dataframe
class_model_pred2 <- pred_grid2 |>
  mutate(
    fit = pred$fit,
    se_fit = pred$se.fit,
    p = plogis(fit),  # convert from logit to probability
    p_lower = plogis(fit - 1.96 * se_fit),
    p_upper = plogis(fit + 1.96 * se_fit)
  )

class_model_pred2_filtered <- class_model_pred2 |>
  filter(pctpov %in% c(0, 25, 50, 75))

ggplot(class_model_pred2_filtered, aes(x = pcthisp, y = p, color = as_factor(pctpov))) +
  geom_line(size = 1.2) +
  geom_ribbon(aes(ymin = p_lower, ymax = p_upper, fill = as_factor(pctpov)), alpha = 0.2, color = NA) +
  labs(
    x = "% Hispanic",
    y = "Predicted Probability of SDWA Violation",
    color = "% Poverty",
    fill = "% Poverty",
    title = "Predicted Probability of SDWA Violations by Race and Class"
  ) +
...
This can be seen in my figure below. Communities with high poverty(50%-75%) experience a much larger probability compared to the low poverty communities(0-25%). The predictions get steeper as the percentage of Hispanics increase. Quantitatively, this is supported by the 95% CI for the interaction interaction (0.00019–0.00046) .
